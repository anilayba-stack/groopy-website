...
import { SERVICE_SLUG_EN_TO_TR } from "@/lib/i18n-routes";
import { getService, serviceSlugs } from "@content/en/services";
import { getPost } from "@content/en/blog";
...
export async function generateMetadata({
  ...
  if (!service) return {};
  const trSlug = SERVICE_SLUG_EN_TO_TR[service.slug];
  return buildMetadata({
    title: service.metaTitle,
    description: service.metaDescription,
    path: enRoutes.service(service.slug),
    locale: "en",
    alternatePath: trSlug ? routes.service(trSlug) : undefined,
  });
}

export default async function EnServiceDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const crumbs = [
    ...
    { name: service.navLabel, path: enRoutes.service(service.slug) },
  ];
  ...
  return (
    <>
      <JsonLd
        data={[
          serviceSchema(service, { path: enRoutes.service(service.slug), locale: "en" }),
      ...
      <section className="service-hero relative isolate overflow-hidden border-b border-[var(--color-border)]">
        ...
        <Container className="relative pt-10 pb-16 sm:pb-20 lg:min-h-[440px] lg:pb-24">
          ...
          <div className="max-w-xl">
            ...
            <div className="mt-8">
              <TldrBox label="Summary">{service.tldr}</TldrBox>
            </div>
          </div>
        </Container>
      </section>
      ...
      {service.faq && service.faq.length > 0 ? (
        <Section>
          <div className="max-w-3xl">
            <FaqList items={service.faq} title="Frequently asked questions" />
          </div>
        </Section>
      ) : null}
        ...
        <Section surface>
          ...
          <ul className="divide-y divide-[var(--color-border)] border-y border-[var(--color-border)]">
            {relatedPosts.map((p) => (
              <li key={p.meta.slug}>
                <Link
                  href={enRoutes.post(p.meta.slug)}
